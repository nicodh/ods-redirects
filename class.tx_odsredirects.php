...
//		$hash = t3lib_div::md5int($speakingURIpath);
        ...
        while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
            ...
            if ($row) {
                ...
                if ($row['domain_id']) {
                    ...
                    // All Domains
                    if (!$specific && $prio[$row['mode']] > $prio[$redirect['mode']]){
                        $redirect = $row;
                    }
                }
            }
    ...
    function getLink($redirect, $source = '') {
        ...
        if (strpos($destination, '@')) {
            ...
            // URL
            return $destination;
        } elseif ((strpos($destination, '.') && strpos($destination, '/') === false) || (strpos($destination,
                                                                                                '.') < strpos($destination,
                                                                                                              '/'))
        ) {
            // URL without http://
...
if (defined('TYPO3_MODE') && $GLOBALS['TYPO3_CONF_VARS']['TYPO3_MODE']['XCLASS']['ext/ods_redirects/class.tx_odsredirects.php']) {
    include_once($GLOBALS['TYPO3_CONF_VARS']['TYPO3_MODE']['XCLASS']['ext/ods_redirects/class.tx_odsredirects.php']);
}
?>